...
`ChromaPatentTool`은 `migration_tmp/load_kipris_to_chroma.py`로 ChromaDB에 적재한 KIPRIS patent collection을 조회하는 RAG 도구입니다.
실제 ChromaDB 연결과 v2 REST API 호출은 `backend/app/storage/chormaDB.py`의 `ChromaDBConnection`이 담당하고,
...
`migration_tmp/load_kipris_to_chroma.py`는 MongoDB row를 다음 형태로 ChromaDB에 저장합니다.
...
- `ChromaDBConnection`
  ...
  - `count()`, `get_documents(...)`, `query_embeddings(...)`로 실제 ChromaDB v2 REST API를 호출합니다.
  - 기본 tenant/database/collection은 migration script와 같습니다.
- `ChromaPatentTool`
